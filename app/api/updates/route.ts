    ...
    try {
        ...
        for (const repoName of repos) {
            ...
            commitData.forEach((commit: any) => {
                // Choose the most reliable timestamp
                const iso =
        ...
        // Sort commits by date and keep 5 most recent
        const sortedCommits = commits
            .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
            .slice(0, 5)

        console.log(`[GitHub] ${sortedCommits.length} commits fetched.`)
    ...
    try {
      ...
      `,
        ...
        if (posts.length > 0) {
            ...
            console.log(`[Hashnode] ${posts.length} post(s) fetched.`);
        } else {
            console.warn("[Hashnode] No posts found.");
        }
    } catch (error) {
